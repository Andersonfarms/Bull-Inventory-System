...
APP_CONFIG = {
    "company_name": "Bull", # e.g., "Apex Logistics", "Smith Farms"
    "app_title": "Inventory System",
    "logo_path": "bull.png", # Swap with client's logo file
    "timezone": "US/Central",
    
    # Database Tables
    "table_inventory": "client_inventory",        # Formerly bull_inventory
    "table_activity": "client_activity_log",      # Formerly bull_activity_log
    "table_inbound": "client_inbound_tracking",   # Formerly bull_inbound_tracking
    ...
    if update_id != "None":
        if st.button("Mark as Arrived"):
            try:
                ...
                st.success(f"✅ Container {update_id} marked as Arrived.")
                st.rerun()
