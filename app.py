...
with tab2:
    ...
    if not attach_df.empty:
        # Group by Model and Size for better clarity
        a_counts = attach_df.groupby(['Model', 'Size'])['Qty_On_Hand'].sum()
        for model_size, count in a_counts.items():
            label = f"{model_size[1]} {model_size[0]}" if model_size[1] else model_size[0]
